 ...
 * under the License.
 */
package org.apache.shiro.web.servlet.osgi;
...
/**
 * Primary Shiro Filter for web applications configuring Shiro via OSGi.
 * <p/>
 * This filter is functionally identical to ShiroFilter, but does not require a WebEnvironment, instead relying on the OSGi container to
 * supply a configured Shiro environent.
 * <h2>Usage</h2>
 ...
 * @since 1.2
 */
public class OSGiShiroFilter extends AbstractShiroFilter {
	...
	BundleContext ctx = BundleReference.class.cast(OSGiShiroFilter.class.getClassLoader())
         .getBundle()
         .getBundleContext();
	
        OSGiAdapter<SecurityManager> adapter = new OSGiAdapter<SecurityManager>(ctx, SecurityManager.class);
	

        setSecurityManager(adapter);
