...
public class OSGiSecurityManager extends DefaultSecurityManager{
    ...
    @Reference(updated = "updatedSubjectDAO", unbind = "unbindSubjectDAO", policy = ReferencePolicy.DYNAMIC, cardinality = ReferenceCardinality.MANDATORY, policyOption = ReferencePolicyOption.GREEDY)
    public void bindSubjectDAO(SubjectDAO sm){
	setSubjectDAO(sm);
    }
    
    public void updatedSubjectDAO(SubjectDAO sm){
	setSubjectDAO(sm);
    }
	
    public void unbindSubjectDAO(SubjectDAO sm){
	setSubjectDAO(null);
    }
